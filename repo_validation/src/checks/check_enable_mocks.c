...
static int is_define_enable_mocks(const char* line, size_t len)
{
    ...
    else
    {
        ...
        while (p < end && (*p == ' ' || *p == '\t')) p++;
        // Check "define"
        if (end - p < 6 || strncmp(p, "define", 6) != 0)
        {
            result = 0;
        }
        else
        {
            ...
            else
            {
                while (p < end && (*p == ' ' || *p == '\t')) p++;
                // Check "ENABLE_MOCKS"
                if (end - p < 12 || strncmp(p, "ENABLE_MOCKS", 12) != 0)
                {
                    result = 0;
                }
                else
...
static int is_undef_enable_mocks(const char* line, size_t len)
{
    ...
    else
    {
        p++;
        while (p < end && (*p == ' ' || *p == '\t')) p++;
        if (end - p < 5 || strncmp(p, "undef", 5) != 0)
        {
            result = 0;
        }
        else
...
static int enable_mocks_check_file(const FILE_INFO* file, const VALIDATOR_CONFIG* config)
{
    ...
    if (violations > 0)
    {
        if (config->fix_mode)
        {
            ...
            else
            {
                    ...
                    if (i == len || content[i] == '\n')
                    {
                        ...
                        if (line_len > 0 && !has_force_comment(line_start, line_len))
                        {
                            if (is_define_enable_mocks(line_start, line_len))
                            {
                                ...
                                (void)memcpy(new_content + out_pos, replacement, rlen);
                                out_pos += rlen;
                                replaced = 1;
                            }
                            else if (is_undef_enable_mocks(line_start, line_len))
                        ...
                        if (!replaced)
                        {
                            (void)memcpy(new_content + out_pos, line_start, raw_line_len);
                ...
                else
                {
                    ...
                    (void)fclose(f);
                    (void)printf("  [FIXED] %s - replaced %d deprecated pattern(s)\n", file->relative_path, violations);
                }
                free(new_content);
                result = 1;
            }
        ...
        {
            (void)printf("  [ERROR] %s - %d deprecated ENABLE_MOCKS pattern(s)", file->relative_path, violations);
            if (define_count > 0)
            {
                (void)printf(" (#define: %d)", define_count);
            }
            else
            {
                /* do nothing */
            ...
            if (undef_count > 0)
            {
                (void)printf(" (#undef: %d)", undef_count);
            }
            else
            {
                /* do nothing */
...
static int enable_mocks_finalize(const VALIDATOR_CONFIG* config)
{
    int result;
    (void)config;
    result = enable_mocks_violations;
    return result;
}
