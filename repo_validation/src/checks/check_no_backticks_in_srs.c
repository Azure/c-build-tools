    ...
    if (match_count > 0)
    {
        if (config->fix_mode)
        {
            ...
            else
            {
                ...
                {
                    (void)fwrite(fixed, 1, new_len, f);
                    (void)fclose(f);
                    (void)printf("  [FIXED] %s - removed backticks from %d SRS requirement(s)\n",
        ...
        {
            (void)printf("  [ERROR] %s - %d SRS requirement(s) contain backticks\n",
                   file->relative_path, match_count);
