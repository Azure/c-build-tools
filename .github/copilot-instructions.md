...
- **Verify with leak detection (VLD) before claiming a test passes**: A plain Debug run that reports `N tests, 0 failed` is NOT sufficient verification — it does not exercise memory-leak detection, but CI does (Visual Leak Detector, and AppVerifier). Before concluding a unit test passes, reconfigure with `-Duse_vld=ON`, rebuild the affected `*_exe_{project}` target, and run it; a clean run must end with `No memory leaks detected`. Treat a leak as a real test failure (do not ignore it). Note: `use_vld=ON` is incompatible with `fsanitize_address=ON`.
