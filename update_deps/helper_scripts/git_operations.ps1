...
function collect-upstream-changes
{
    ...
    $changes = @()
    $seen_shas = @{}
    Write-Verbose "collect-upstream-changes: repo=$repo_name"
            ...
            foreach ($line in $submodule_lines)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            Write-Verbose "  ${sub_repo_name}: no fixed commit, skipping"
                        }
                        ...
                        if ($new_sha -and $old_sha -ne $new_sha)
                        {
                            ...
                            {
                                Write-Verbose "  ${sub_repo_name}: no commits between $($old_sha.Substring(0,8))..$($new_sha.Substring(0,8))"
                            }
...
function update-local-repo
{
    ...
    if ((-not $upstream_changes -or $upstream_changes.Count -eq 0))
    {
        ...
        if ($staged_files)
        {
            ...
            # Fallback: if no changes were found from submodule diffs above (e.g., all
            # commits were filtered as dep-updates), try extracting the c-build-tools
            # commit log from YAML ref changes. The old/new SHAs in the YAML ref: field
            # give us the same range that the submodule diff would have provided.
            $yaml_changes = @($changed_files | Where-Object { $_ -match "^build/.*\.yml$" })
            if ($yaml_changes.Count -gt 0)
            {
                # Extract old/new SHA from the staged diff of the first changed YAML
                $yaml_diff = git diff --cached -U0 -- $yaml_changes[0] 2>$null
                $old_ref = $null
                $new_ref = $null
                if ($yaml_diff)
                {
                    foreach ($dl in $yaml_diff)
                    {
                        if ($dl -match "^-\s*ref:\s*([0-9a-f]{40})")
                        {
                            $old_ref = $matches[1]
                        }
                        if ($dl -match "^\+\s*ref:\s*([0-9a-f]{40})")
                ...
                if ($old_ref -and $new_ref -and $old_ref -ne $new_ref)
                {
                    # Try to get commit log from c-build-tools submodule or repo
                    $cbt_path = $null
                    if (Test-Path "deps/c-build-tools") { $cbt_path = "deps/c-build-tools" }
                    elseif (Test-Path "deps/c-build-tools-repo") { $cbt_path = "deps/c-build-tools-repo" }

                    if ($cbt_path)
                    {
                        Push-Location $cbt_path
                        git fetch origin 2>$null
                        $cbt_log = git log --oneline "$old_ref..$new_ref" 2>$null
                        if ($cbt_log)
                        {
                            # Replace fallback entries with actual commit log
                            $upstream_changes = @()
                            foreach ($cbt_line in $cbt_log)
                            {
                                if ($cbt_line -match "^([0-9a-f]+)\s+(.+)$")
                                {
                                    $cbt_sha = $matches[1]
                                    $cbt_subject = $matches[2]

                                    # Filter out dep-update commits
                                    if ($cbt_subject -eq "Update dependencies" -or
                                        $cbt_subject -like "Update deps:*" -or
                                        $cbt_subject -like '`[autogenerated`]*')
                                    {
                                        # skip
                                    }
                                    else
                                    {
                                        # Replace GitHub PR refs with full URLs
                                        if ($cbt_subject -match '\(#(\d+)\)')
                                        {
                                            $cbt_subject = [regex]::Replace($cbt_subject, '\(#(\d+)\)', { param($m) "(https://github.com/Azure/c-build-tools/pull/$($m.Groups[1].Value))" })
                                        }
                                        $upstream_changes += @{
                                            Repo = "c-build-tools"
                                            SHA = $cbt_sha
                    ...
                    # Also include recursive upstream changes from c-build-tools
                    if ($global:repo_change_descriptions -and
                        $global:repo_change_descriptions.ContainsKey("c-build-tools"))
                    {
                        $seen_fallback = @{}
                        foreach ($c in $upstream_changes) { if ($c.SHA) { $seen_fallback[$c.SHA] = $true } }
                        foreach ($uc in $global:repo_change_descriptions["c-build-tools"])
                        {
                            if ($uc.SHA -and -not $seen_fallback.ContainsKey($uc.SHA))
                            {
                                $seen_fallback[$uc.SHA] = $true
                                $upstream_changes += $uc
                            }
                        }
                    }
