                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if ($new_sha -and $old_sha -ne $new_sha)
                        {
                            ...
                            Write-Verbose "  submodule ${sub_path}: git log returned $(@($log_lines).Count) lines"
                            if ($log_lines)
                            {
                                foreach ($log_line in $log_lines)
                                {
                                    if ($log_line -match "^([0-9a-f]+)\s+(.+)$")
                                    {
                                        ...
                                        {
                                            Write-Verbose "  submodule ${sub_path}: filtered dep-update commit $commit_sha"
                                        }
                                        else
                                        {
                                            if (-not $seen_shas.ContainsKey($commit_sha))
                                            {
                                                ...
                                                Write-Verbose "  submodule ${sub_path}: added change $commit_sha '$commit_subject'"
                                            }
                                            else
                                            {
                                                Write-Verbose "  submodule ${sub_path}: duplicate SHA $commit_sha, skipping"
                                            }
                                        }
                                    }
                                    else
                                    {
                                        Write-Verbose "  submodule ${sub_path}: couldn't parse log line: '$log_line'"
                                    }
                                }
                            }
                            else
                            {
                                Write-Verbose "  submodule ${sub_path}: no commits in range $old_sha..$new_sha"
                            }
