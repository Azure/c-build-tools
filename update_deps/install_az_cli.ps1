...
function check-az-cli-exists {
    ...
    if($LASTEXITCODE -ne 0) {
        Write-Host "Azure DevOps extension appears corrupted. Reinstalling..."
        az extension remove --name azure-devops 2>$null
        $reinstallOutput = az extension add --name azure-devops 2>&1
        if($LASTEXITCODE -ne 0) {
            $reinstallError = $reinstallOutput | Out-String
            Write-Error "Failed to reinstall azure-devops extension: $reinstallError"
            Write-Error "See: https://learn.microsoft.com/en-us/azure/devops/cli/"
            exit -1
        }
        Write-Host "Azure DevOps extension reinstalled successfully." -ForegroundColor Green
    }
    ...
    if($LASTEXITCODE -ne 0) {
        Write-Host "WAM authentication succeeded but cannot access Azure DevOps. Attempting interactive Azure DevOps login..."
        # Use az devops login with interactive browser authentication
        az devops login --org "https://dev.azure.com/msazure"
        if($LASTEXITCODE -ne 0) {
            Write-Error "Azure DevOps login failed. Please provide a PAT token using -azure_token parameter."
            exit -1
        }
        # Verify again after login
        $testResult = az devops project list --org "https://dev.azure.com/msazure" --top 1 -o json 2>$null
        if($LASTEXITCODE -ne 0) {
            Write-Error "Still cannot access Azure DevOps after login. Please provide a PAT token using -azure_token parameter."
            exit -1
        }
        Write-Host "Azure DevOps authentication successful." -ForegroundColor Green
    }
}
