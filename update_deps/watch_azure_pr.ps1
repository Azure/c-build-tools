...
param(
    [Parameter(Mandatory=$true)][int]$pr_id,
...
function Get-PolicyDisplayData {
    param(
        [int] $pr_id,
        [string] $org,
    ...
    # Fetch policies
    $policies = Get-AzurePRPolicies -pr_id $pr_id -org $org
    if(!$policies) {
        return $null
    }
    ...
    if($ShowBuildDetails) {
        foreach($policy in $policies) {
            if($policy.BuildId -and $policy.ProjectId) {
                $jobs = Get-BuildJobDetails -build_id $policy.BuildId -project_id $policy.ProjectId -org $org
                if($jobs) {
...
function Watch-AzurePRPolicies {
    param(
        [int] $pr_id,
        [string] $org,
        [int] $poll_interval = 30,
    ...
    while($true) {
        ...
        $displayData = Get-PolicyDisplayData -pr_id $pr_id -org $org -ShowBuildDetails:$ShowBuildDetails
...
function Show-AzurePRPolicyStatus {
    param(
        [int] $pr_id,
        [string] $org,
        [switch] $ShowBuildDetails
    ...
    $displayData = Get-PolicyDisplayData -pr_id $pr_id -org $org -ShowBuildDetails:$ShowBuildDetails
    if($displayData) {
        Show-PolicyStatus -displayData $displayData -pr_id $pr_id -poll_interval 30 -ShowBuildDetails:$ShowBuildDetails
    }
}
...
if($MyInvocation.InvocationName -ne '.') {
    $success = Watch-AzurePRPolicies -pr_id $pr_id -org $org -poll_interval $poll_interval -timeout $timeout -ShowBuildDetails
    if($success) {
        exit 0
    } else {
        exit 1
