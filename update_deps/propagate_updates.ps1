...
# create work item that will be linked to the pull request
function create-work-item-azure {
    $request_url = "https://dev.azure.com/msazure/One/_apis/wit/workitems/`$Task?api-version=6.0"
    $body = '[
        {
          "op": "add",
          "path": "/fields/System.Title",
          "from": null,
          "value": "[autogenerated] Update dependencies"
        }
      ]'
    $response = Invoke-WebRequest -URI $request_url -UseBasicParsing -Method Post -Headers $azure_header -Body $body -ContentType "application/json-patch+json"
    if(!$response) {
